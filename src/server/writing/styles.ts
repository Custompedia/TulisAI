...
type StyleRow = { id: string; name: string; color: string | null; icon: string | null; settings_json: string; created_at: number; updated_at: number };
const now = () => Date.now();
...
const dto = (row: StyleRow): WritingStyle => ({ id: row.id, name: row.name, color: isNotebookColor(row.color) ? row.color : null, icon: row.icon, settings: styleSettings(parse(row.settings_json)), createdAt: new Date(row.created_at).toISOString(), updatedAt: new Date(row.updated_at).toISOString() });
const SELECT = "SELECT id,name,color,icon,settings_json,created_at,updated_at FROM writing_styles";
...
export async function createStyle(ownerId: string, input: StyleInput): Promise<WritingStyle> {
  ...
  try {
    const result = await runtime().DB.prepare("INSERT INTO writing_styles (id,owner_id,name,color,icon,settings_json,created_at,updated_at) SELECT ?,?,?,?,?,?,?,? WHERE (SELECT COUNT(*) FROM writing_styles WHERE owner_id=?)<?").bind(id, ownerId, input.name, input.color, input.icon, settings, created, created, ownerId, STYLE_LIMIT).run();
  ...
  return dto({ id, name: input.name, color: input.color, icon: input.icon, settings_json: settings, created_at: created, updated_at: created });
}
...
export async function updateStyle(ownerId: string, styleId: string, changes: StylePatch): Promise<WritingStyle> {
  ...
  const next: StyleRow = { ...existing, name: changes.name ?? existing.name, color: changes.color === undefined ? existing.color : changes.color, icon: changes.icon === undefined ? existing.icon : changes.icon, settings_json: changes.settings === undefined ? existing.settings_json : JSON.stringify(styleSettings(changes.settings)), updated_at: updated };
  try { await runtime().DB.prepare("UPDATE writing_styles SET name=?,color=?,icon=?,settings_json=?,updated_at=? WHERE id=? AND owner_id=?").bind(next.name, next.color, next.icon, next.settings_json, updated, styleId, ownerId).run(); }
  catch (error) { if (isUnique(error)) throw new RequestError("STYLE_EXISTS", "A style with this name already exists.", 409); throw error; }
  return dto(next);
}
