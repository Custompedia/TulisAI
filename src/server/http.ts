...
// Binary upload body, read with a hard cap and no buffering past it, so an oversized or lying
// content-length cannot be used to exhaust the worker.
export async function readBinary(request: Request, maxBytes: number): Promise<Uint8Array> {
  ...
  try {
    while (true) {
      ...
      if (bytes > maxBytes) { await reader.cancel(); throw new RequestError("PAYLOAD_TOO_LARGE", "The file is too large.", 413); }
