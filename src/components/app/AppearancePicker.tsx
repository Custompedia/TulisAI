...
export function AppearancePicker({ anchor, color, icon, mode, onSelect, onClose }: { anchor: HTMLElement | null; color: string | null; icon: string | null; mode: string | null; onSelect: (next: NotebookAppearance) => void; onClose: () => void }) {
  ...
  useEffect(() => { if (placed) (search.current ?? panel.current?.querySelector<HTMLButtonElement>('[role="tab"][aria-selected="true"]'))?.focus({ preventScroll: true }); }, [placed]);
  ...
  return createPortal(
    <div ref={panel} role="dialog" aria-label={t('Ubah ikon & warna', 'Change icon & colour')} onClick={(event) => event.stopPropagation()}
      ...
      <div className="flex items-center gap-1 border-b border-line px-2 pt-2 pb-1.5">
        ...
        <button type="button" onClick={() => onSelect({ color: null, icon: null })} disabled={!color && !icon} className="ml-auto h-8 rounded-md px-2 text-[13px] font-medium text-ink-500 outline-none hover:text-red-700 focus-visible:ring-2 focus-visible:ring-brand-400 disabled:opacity-40 disabled:hover:text-ink-500">{t('Hapus', 'Remove')}</button>
        ...
        {tab === 'emoji' ? (
          <>
            <div className="relative mb-2">
              ...
              <input ref={search} type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('Cari emoji…', 'Search emoji…')} aria-label={t('Cari emoji', 'Search emoji')}
            ...
            {emoji.length === 0 ? <p className="px-2 py-6 text-center text-[13px] text-ink-500">{t(`Tidak ada emoji untuk “${query.trim()}”.`, `No emoji for “${query.trim()}”.`)}</p> : (
              <div onKeyDown={gridKeys(EMOJI_COLS)} className="scrollbar-thin grid max-h-52 grid-cols-8 overflow-y-auto">
          ...
          <div onKeyDown={gridKeys(ICON_COLS)} className="grid grid-cols-7 justify-items-center gap-y-1">
