...
export const CHIP = 'inline-flex h-9 shrink-0 items-center gap-1.5 rounded-full border border-line-strong bg-white px-3.5 text-[13px] text-ink-800 transition-colors hover:border-ink-300 hover:text-ink-900 disabled:opacity-50';
const PANEL = 'absolute top-full z-40 mt-2 max-w-[calc(100vw-2rem)] rounded-2xl border border-line bg-white p-1.5 shadow-[0_16px_40px_-12px_rgb(31_32_29/0.25)] animate-fade-up';
  ...
  return (
    <div ref={root} className="relative">
      <button ref={button} type="button" aria-haspopup="true" aria-expanded={open} aria-controls={open ? id : undefined} disabled={disabled} onClick={() => setOpen(!open)}
        className={ghost ? `inline-flex h-9 items-center gap-1 rounded-full px-2.5 text-[13px] font-semibold text-ink-600 transition-colors hover:bg-paper-deep hover:text-ink-900 disabled:opacity-50 ${open ? 'bg-paper-deep text-ink-900' : ''}` : `${CHIP} ${open ? 'border-ink-300 text-ink-900' : ''}`}>
        {!ghost && <span className="font-semibold">{label}</span>}
...
export function ChipSelect<T extends string>({ label, value, options, onChange, disabled, ghost, align }: { label: string; value: T; options: Array<Option<T>>; onChange: (value: T) => void; disabled?: boolean; ghost?: boolean; align?: 'start' | 'end' }) {
  return (
    <ChipPopover label={label} value={options.find((option) => option.value === value)?.label} disabled={disabled} ghost={ghost} align={align} width="w-56">
      {(close) => (
          ...
          {options.map((option) => {
            ...
            return (
              <button key={option.value} type="button" role="option" aria-selected={selected} disabled={option.disabled} onClick={() => { onChange(option.value); close(); }}
                className={`flex h-10 w-full items-center gap-2 rounded-xl px-3 text-left text-[14px] transition-colors disabled:opacity-40 ${selected ? 'bg-brand-50 font-semibold text-ink-900' : 'text-ink-700 hover:bg-paper'}`}>
                <span className="min-w-0 flex-1 truncate">{option.label}</span>{selected && <Check size={16} aria-hidden="true" className="shrink-0 text-brand-700" />}
              </button>
            );
          })}
        </div>
...
export function ChipText({ label, value, placeholder, maxLength, onChange, disabled }: { label: string; value: string; placeholder: string; maxLength: number; onChange: (value: string) => void; disabled?: boolean }) {
  const { t } = useLocale();
  return (
    <ChipPopover label={label} value={value || t('Opsional', 'Optional')} disabled={disabled} width="w-72">
      {(close) => (
        <form className="space-y-2 p-1.5" onSubmit={(event) => { event.preventDefault(); close(); }}>
          <input autoFocus aria-label={label} value={value} maxLength={maxLength} placeholder={placeholder} onChange={(event) => onChange(event.target.value)}
            className="h-10 w-full rounded-xl border border-line-strong bg-white px-3 text-[14px] text-ink-900 placeholder:text-ink-400 focus:border-brand-400 focus:outline-none focus:ring-3 focus:ring-brand-100" />
