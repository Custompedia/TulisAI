...
export const inputClass = 'h-10 w-full rounded-lg border border-line-strong bg-white px-3 text-sm text-ink-900 placeholder:text-ink-300 transition-colors hover:border-ink-300 focus:border-brand-500 focus:outline-none focus:ring-3 focus:ring-brand-100 disabled:bg-paper disabled:text-ink-400';
...
export function Select({ value, onChange, options, id, label, disabled, className = '' }: { value: string; onChange: (value: string) => void; options: Array<{ value: string; label: string; disabled?: boolean }>; id?: string; label?: string; disabled?: boolean; className?: string }) {
  return (
    <div className={`relative ${className}`}>
      <select id={id} aria-label={id ? undefined : label} value={value} disabled={disabled} onChange={(event) => onChange(event.target.value)} className={`${inputClass} appearance-none pr-9`}>
        {options.map((option) => <option key={option.value} value={option.value} disabled={option.disabled}>{option.label}</option>)}
      </select>
      <ChevronDown size={16} aria-hidden="true" className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-ink-400" />
    </div>
  );
}
