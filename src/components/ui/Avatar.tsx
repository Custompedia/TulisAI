...
export function AvatarGlyph({ className = '' }: { className?: string }) {
  return (
    <svg viewBox="0 0 64 64" aria-hidden="true" focusable="false" className={`h-full w-full text-brand-600 ${className}`}>
      <circle cx="32" cy="24.5" r="11.5" fill="currentColor" />
      <path d="M9 64c0-13.8 10.3-24 23-24s23 10.2 23 24Z" fill="currentColor" fillOpacity="0.82" />
    </svg>
  );
}
...
export function Avatar({ name, image, size = 36, className = '' }: { name: string; image?: string | null; size?: number; className?: string }) {
  ...
  return (
    <span role="img" aria-label={name} className={`relative inline-block shrink-0 overflow-hidden rounded-full bg-brand-50 ring-1 ring-brand-100 ${className}`} style={{ width: size, height: size }}>
      {showImage
