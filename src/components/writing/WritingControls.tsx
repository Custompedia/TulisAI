...
import { defaults, EXTRA_LIMIT, type Mode, type Settings, type Strength } from '@/lib/writing/settings';
...
import {
  ...
  MODES, modeHint, modeIcon, modeLabel, preservationOptions, requestSummary, strengthOptions,
} from './modes';
      ...
      {modes.map((mode) => {
        ...
        return (
          <button key={mode} type="button" role="radio" aria-checked={active} disabled={disabled} title={modeHint(mode, t)} onClick={() => onChange(mode)}
            className={`group inline-flex items-center gap-2 rounded-lg border text-left font-semibold transition-colors disabled:opacity-50 ${layout === 'grid' ? 'h-10 px-2.5 text-[13px]' : 'h-9 px-3.5 text-[13px]'} ${active ? 'border-brand-600 bg-brand-600 text-white' : 'border-line-strong bg-white text-ink-700 hover:border-ink-300 hover:text-ink-900'}`}>
            <Icon size={15} aria-hidden="true" className={active ? 'text-white' : 'text-ink-400 group-hover:text-ink-600'} />
...
export function CustomizePanel({ settings, onChange, disabled, defaultOpen = false }: { settings: Settings; onChange: (settings: Settings) => void; disabled?: boolean; defaultOpen?: boolean }) {
  const { t } = useLocale();
  const id = useId();
  const [open, setOpen] = useState(defaultOpen || settings.mode === 'custom');
  ...
  const apply = () => { onChange({ ...settings, ...draft, audience: draft.audience.trim() || 'general_public', customized: true }); setOpen(false); };
  ...
  return (
    <div className={`rounded-xl border ${open ? 'border-line-strong bg-white' : 'border-line bg-white/60'}`}>
      <button type="button" onClick={toggle} aria-expanded={open} aria-controls={`${id}-panel`} className="flex w-full items-center gap-2.5 px-3.5 py-2.5 text-left">
        ...
        {active && !open && <span className="rounded-md bg-brand-50 px-2 py-0.5 text-[11px] font-semibold text-brand-700">{t('Aktif', 'On')}</span>}
        <ChevronDown size={16} className={`text-ink-400 transition-transform ${open ? 'rotate-180' : ''}`} aria-hidden="true" />
      </button>
      {open && (
        <div id={`${id}-panel`} className="space-y-4 border-t border-line px-3.5 pb-4 pt-3.5">
          ...
          <fieldset>
            <legend className="mb-1.5 text-[13px] font-semibold text-ink-700">{t('Fokus', 'Focus')} <span className="font-normal text-ink-400">({t('opsional', 'optional')})</span></legend>
            <div className="flex flex-wrap gap-1.5">
              {focusOptions(t).map((option) => {
                const on = draft.focus.includes(option.value);
                return (
                  <button key={option.value} type="button" aria-pressed={on} disabled={disabled} onClick={() => setDraft({ ...draft, focus: on ? draft.focus.filter((value) => value !== option.value) : [...draft.focus, option.value] })}
                    className={`inline-flex h-8 items-center gap-1.5 rounded-lg border px-2.5 text-xs font-semibold transition-colors ${on ? 'border-brand-300 bg-brand-50 text-brand-800' : 'border-line-strong bg-white text-ink-600 hover:border-ink-300'}`}>
          ...
          <div>
            ...
            <textarea id={`${id}-extra`} rows={2} maxLength={EXTRA_LIMIT} disabled={disabled} value={draft.extra} onChange={(event) => setDraft({ ...draft, extra: event.target.value })}
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex gap-2">
              <Button size="sm" icon={X} onClick={() => setOpen(false)}>{t('Tutup', 'Close')}</Button>
      ...
      {!open && active && <p className="border-t border-line px-3.5 py-2 text-xs leading-relaxed text-ink-500">{requestSummary(settings, t)}</p>}
    </div>
  );
}
