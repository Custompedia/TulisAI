...
function Faq() {
  ...
  const items = [
    ...
    [t('Bahasa apa yang didukung?', 'Which languages are supported?'), t('Bahasa Indonesia dan Inggris. Kamu juga bisa mengganti bahasa antarmuka melalui pilihan ID atau EN di bagian atas halaman.', 'Indonesian and English. You can also switch the interface language using the ID or EN options at the top of the page.')],
  ...
  return <section id="faq" className={`${styles.container} ${styles.section} ${styles.faqSection}`}><Heading title={t('Ada pertanyaan?\nMari kita perjelas.', 'Questions?\nLet’s clear things up.')} description={t('Kenali ruang menulismu sedikit lebih dekat.', 'Get to know your writing space a little better.')} /><div className={styles.faqList}>{items.map(([question, answer], index) => <details key={question} name="landing-faq" open={index === 0 ? true : undefined}><summary><span className={styles.faqPlus}><Plus size={15} /></span><span>{question}</span></summary><div className={styles.faqAnswer}><p>{answer}</p><span className={styles.answerIcon} aria-hidden="true"><PenLine size={14} /></span></div></details>)}</div></section>;
}
