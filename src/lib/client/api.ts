...
export function errorText(error: unknown, english: boolean): string {
  ...
  switch (true) {
    ...
    // Purchases. Every message says what did not change, because MKL, not this page, decides what was paid.
    case code === 'COMMERCE_CHECKOUT_CLOSED': return t('Pembelian belum dibuka. Paket dan kuotamu tidak berubah.', 'Purchases are not open yet. Your plan and allowance are unchanged.');
    case code === 'MKL_COMMERCE_NOT_CONFIGURED': return t('Pembayaran lewat MKL belum disiapkan di aplikasi ini.', 'Payment through MKL is not set up in this app yet.');
    case code === 'MKL_IDENTITY_REQUIRED': return t('Tautkan akun MKL dulu dari Pengaturan untuk membeli.', 'Link your MKL account in Settings before buying.');
    case code === 'PURCHASE_IDENTITY_MISMATCH': return t('Akun MKL yang dipakai berbeda dengan yang tertaut ke TulisAI. Masuk ke MKL dengan akun yang sama, lalu coba lagi.', 'The MKL account used differs from the one linked to TulisAI. Sign in to MKL with the same account and try again.');
    case code === 'PURCHASE_IDENTITY_CHANGED': return t('Pembelian ini milik identitas MKL yang sudah tidak tertaut ke akun ini.', 'This purchase belongs to an MKL identity that is no longer linked to this account.');
    case code === 'ACTIVE_ACCESS_EXISTS': return t('Paketmu masih aktif. Paket baru bisa dibeli setelah periode ini berakhir.', 'Your plan is still active. A new plan can be bought after this period ends.');
    case code === 'FRESH_PAID_ACCESS_REQUIRED': return t('Tambahan karakter hanya untuk paket Plus, Pro, atau Max yang sedang aktif.', 'Character top-ups are only for an active Plus, Pro, or Max plan.');
    case code === 'PURCHASE_ALREADY_OPEN': return t('Masih ada pembelian yang belum selesai. Lanjutkan atau periksa statusnya di Pengaturan › Pembelian.', 'A purchase is still in progress. Continue it or check its status in Settings › Purchases.');
    case code === 'BUYER_PHONE_INVALID': return t('Nomor HP tidak valid.', 'The phone number is invalid.');
    case code === 'OFFER_CHANGED': return t('Penawaran berubah sejak kamu memulai. Tidak ada yang ditagih; mulai pembelian baru.', 'The offer changed since you started. Nothing was charged; start a new purchase.');
    case ['OFFER_NOT_AVAILABLE', 'OFFER_AMBIGUOUS', 'OFFER_CONTRACT_MISMATCH', 'OFFER_PROVENANCE_MISMATCH', 'UNSUPPORTED_PRODUCT'].includes(code): return t('Paket ini sedang tidak dijual. Tidak ada yang berubah; coba lagi nanti.', 'This plan is not on sale right now. Nothing changed; try again later.');
    case code === 'PURCHASE_NOT_FOUND' || code === 'PURCHASE_RETURN_NOT_BOUND': return t('Pembelian tidak ditemukan di browser ini. Periksa di Pengaturan › Pembelian.', 'The purchase was not found in this browser. Check Settings › Purchases.');
    case code === 'PURCHASE_NOT_AUTHORIZABLE': return t('Pembelian ini tidak bisa dilanjutkan lagi. Mulai pembelian baru bila perlu.', 'This purchase cannot be continued. Start a new one if needed.');
    case code === 'PURCHASE_IDEMPOTENCY_CONFLICT' || code === 'PURCHASE_IDEMPOTENCY_KEY_INVALID': return t('Permintaan pembelian bentrok. Tutup lalu buka lagi paketnya, kemudian coba lagi.', 'The purchase request clashed. Close and reopen the plans, then try again.');
    case code.startsWith('PURCHASE_') || code.startsWith('ORDER_') || code === 'MKL_CORRECTION_SEQUENCE_INVALID' || code === 'LOCAL_INTENT_CORRUPT': return t('Status pembelian belum bisa dipastikan. Pembelian tetap tersimpan dan akan dicocokkan ulang dengan MKL; paket dan kuotamu belum diubah.', 'The purchase status could not be confirmed. It stays recorded and will be checked with MKL again; your plan and allowance are unchanged.');
    case code.startsWith('MKL_'): return t('MKL belum bisa memproses pembelian. Tidak ada yang berubah; coba lagi nanti.', 'MKL could not process the purchase. Nothing changed; try again later.');
