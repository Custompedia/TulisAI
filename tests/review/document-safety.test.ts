...
describe('review: actual AI pipeline with mocked provider transport',()=>{
  ...
  it('records v3 runtime metadata and flags P03 output above the preservation ceiling',async()=>{
    ...
    expect(row.prompt_version).toBe('v5');expect(JSON.parse(row.runtime_json)).toMatchObject({prompt_version:'v5',reasoning_effort:'low',humanizer_context:'umum',preservation:'conservative'});
  });
