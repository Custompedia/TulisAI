...
| T17 | Penggaris halaman: margin kiri/kanan, indent kiri/kanan/baris pertama, dan tab stop bisa digeser per paragraf — penanda mengikuti kursor lewat langganan transaksi, dan angkanya dihitung dari margin kiri seperti Word, bukan dari tepi kertas | Selesai lokal; belum diverifikasi visual |
...
| T20 | Garis paragraf (`w:pBdr`) dua arah: garis di bawah judul bagian tidak lagi hilang saat impor; paragraf kosong bergaris tetap menjadi garis horizontal; urutan anak `w:pPr` mengikuti skema | Selesai lokal |
| T21 | Tab stop digambar sungguhan: lebar tiap tab dihitung dari lebar teks di antaranya (diukur lewat Range, jadi baris yang sudah wrap tetap terukur) lalu ditempatkan pada stop kiri/tengah/kanan atau kisi 0,5 inci — inilah yang membuat tanggal rata kanan berhenti di margin | Selesai lokal; belum diverifikasi visual |
| T22 | Tab tidak lagi memindahkan fokus keluar editor: Tab menggeser paragraf/daftar ke kanan, Shift+Tab sebaliknya, di tengah teks menyisipkan karakter tab, di dalam tabel tetap pindah sel; perintahnya satu perintah ProseMirror biasa (bukan perintah TipTap di dalam perintah TipTap, yang memicu "applying a mismatched transaction"), Tab selalu dianggap tertangani, dan tombol gutter paragraf keluar dari urutan tab | Selesai lokal |
| T23 | Indentasi daftar mengikuti dokumen: atribut `indent` per level, dan daftar yang hanya lebih menjorok (tanpa naik level penomoran) tetap tersarang seperti di Word | Selesai lokal |

Batas yang diketahui dan disengaja: gambar tetap tidak diimpor atau diekspor; teks berkolom ditampilkan sebagai satu lembar panjang karena mesin paginasi mengukur blok satu kolom; catatan kaki tampil sebagai nomor di kanvas dan baru turun ke kaki halaman di Word; tautan internal Word (`w:hyperlink w:anchor`) masih menjadi teks biasa.
