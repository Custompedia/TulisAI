...
Client ID, app key, catalog item, dan secret produksi belum ditetapkan di source karena commissioning TulisAI belum dilakukan. Untuk test/dev, gunakan nilai test eksplisit dan origin localhost. Endpoint lokal bridge adalah `POST /api/auth/mkl/start`, `POST /api/auth/mkl/link/start`, `POST /api/auth/mkl/commerce/start`, `GET /api/auth/mkl/callback`, serta `GET`/`POST /api/account/mkl/link/confirm`. B5 memakai return URI exact same-origin `${BETTER_AUTH_URL}/api/commerce/mkl/return` secara default; `MKL_COMMERCE_RETURN_URI` hanya diperlukan jika URI terdaftar memakai path same-origin lain, tanpa query/fragment. Pembelian baru hanya terbuka jika var `TULISAI_COMMERCE_CHECKOUT_ENABLED` bernilai tepat `"true"`; default di `wrangler.jsonc` adalah `"false"`. Saat tertutup, recovery, rekonsiliasi entitlement, dan penerapan refund/reversal tetap berjalan. Lihat `docs/b5-mkl-commerce.md`. B6 menambah `GET /api/commerce/offers`, yang menjawab status tiap produk untuk akun ini (tanpa memanggil MKL selama checkout tertutup); UI pembelian hanya merender status itu. Lihat `docs/b6-commercial-frontend.md`.
...
MKL Identity V2 (MKL ADR-016): logout TulisAI memasang cookie `__Host-tulis_mkl_reauth` (hook after `/sign-out`), dan sign-in MKL berikutnya mengirim `prompt=login` supaya sesi MKL yang masih hidup tidak memasukkan akun sebelumnya secara diam-diam; sign-in yang selesai menghapus cookie itu. Setiap link eksplisit selalu mengirim `prompt=login` (kontrol terverifikasi yang segar atas identitas MKL), dan begitu discovery MKL mengiklankan `auth_time`, callback link menolak (`MKL_REAUTH_REQUIRED`) login MKL yang terjadi sebelum ceremony dimulai. Sebelum MKL V2 live, MKL mengabaikan `prompt` dan pemeriksaan `auth_time` tidak aktif, sehingga perilaku sama seperti sebelumnya. Login lokal dan Google langsung milik TulisAI tidak berubah (keputusan OD-4 menunggu owner).
...
Set `BETTER_AUTH_URL` pada vars ke origin Worker/domain target dan daftarkan callback Google yang sama. `MKL_CLIENT_ID`, `MKL_APP_KEY`, dan `MKL_CATALOG_ITEM_ID` baru boleh diisi setelah client/application row disetujui dan diregistrasikan; registrasi redirect/post-logout/commerce return, aktivasi SSO, app API secret, offer, dan commissioning entitlement adalah langkah terpisah. Source B3/B5 bukan izin untuk membuat client, secret, atau offer produksi. Terapkan migrasi dengan `wrangler d1 migrations apply <database_name> --remote` hanya setelah database target diverifikasi. Build dan dry-run dahulu; `pnpm exec wrangler deploy` merupakan langkah eksternal terpisah setelah persetujuan rilis. AI tetap `false` sampai konfigurasi provider, kuota, dan evaluasi hasil disetujui.
